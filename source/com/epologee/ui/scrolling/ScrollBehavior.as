package com.epologee.ui.scrolling {
	...
	public class ScrollBehavior extends EventDispatcher {
		...
		public function set inset(inset : Number) : void {
			_inset = inset;
		}
		...
		private function handleThumbScroll(event : MouseEvent = null) : void {
			var ds : Number;
			if (isVertical()) {
				ds = _thumb.y - _track.y + _inset;
				normalizedValue = ds / _bounds.height;
			} else {
				ds = _thumb.x - _track.x + _inset;
		...
		private function updateFromNormalized(inDispatch : Boolean = true) : void {
			...
			if (isVertical()) {
				_thumb.y = _track.y + normalizedValue * _bounds.height + _inset;
			} else {
				_thumb.x = _track.x + normalizedValue * _bounds.width + _inset;
			}
		...
		private function updateThumbSize() : void {
			if (isVertical()) {
				_thumb.height = Math.min(Math.floor((_track.height - _inset * 2) * _windowSize / _contentSize), _track.height - _inset * 2);
				_bounds = new Rectangle(_thumb.x, _track.y + _inset, 0, Math.ceil(_track.height - _thumb.height - _inset * 2));
			} else {
				_thumb.width = Math.min(Math.floor((_track.width - _inset * 2) * _windowSize / _contentSize), _track.width - _inset * 2);
				_bounds = new Rectangle(_track.x + _inset, _thumb.y, Math.ceil(_track.width - _thumb.width - _inset * 2), 0);
			}
