package com.epologee.animation.trigonometry {
	...
	public class Trigonometry2D {
		...
		/**
		 * @return angle of vector/point in radians
		 */
		public static function angleOfVector(vector : Point) : Number {
		...
		/**
		 * @return a clean angle between 0 and 2 * Math.PI
		 */
		public static function cleanAngle(angleRadians : Number) : Number {
			var fullCircle : Number = 2 * Math.PI;
			
			while (angleRadians < 0) {
				angleRadians += fullCircle;
			}

			while (angleRadians > fullCircle) {
				angleRadians -= fullCircle;
			}
		...
		public static function toRadians(angleDegrees : Number, clean : Boolean = false) : Number {
			var angleRadians : Number = angleDegrees * Math.PI / 180;
			if (clean) angleRadians = cleanAngle(angleRadians);
			return angleRadians;
		}

		public static function toDegrees(angleRadians : Number, clean:Boolean = false) : Number {
			if (clean) angleRadians = cleanAngle(angleRadians);
